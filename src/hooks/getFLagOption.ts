...
export const preloadFlagOptions = (): void => {
  const flagsDir = path.join(process.cwd(), 'public', 'flags')

  if (!fs.existsSync(flagsDir)) {
    console.warn('⚠️ Flags folder not found:', flagsDir)
    return
  }
...
export const getFlagOptions = (): Option[] => {
  if (!loaded) preloadFlagOptions()
  return cachedFlags
}
